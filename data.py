...
def degrade_image(img: np.ndarray, degradation: str = 'gaussian', **kwargs) -> np.ndarray:
    ...
    if degradation == 'gaussian':
        ...
        # ВСЕГДА применяем оба типа: сначала блюр, потом шум
        img = add_gaussian_blur(img, kernel_size=kwargs.get('kernel_size', 5), sigma=kwargs.get('blur_sigma', 0))
        img = add_gaussian_noise(img, sigma=kwargs.get('sigma', 25))
        return img
